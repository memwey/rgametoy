...
/// Length of mode 3 on line 2 (a normal line — line 0 after enable is special),
/// measured from the OAM-scan→drawing edge to the drawing→HBlank edge.
pub fn mode3_len_on_line2(ppu: &mut Ppu) -> u32 {
    while ppu.ly != 2 {
        ppu.tick(1);
    }
...
pub fn mode3_len_with_sprites(sprite_xs: &[u8]) -> u32 {
    let mut ppu = Ppu::new();
