...
fn setup(program: &[u8]) -> Console {
    ...
    console.cpu_mut().set_sp(0xD000); // stack lives in WRAM
    console
}
...
fn test_ld_r_d8_and_ld_r_r() {
    ...
    c.step();
    assert_eq!(c.cpu().get_registers().get_b(), 0x42);
    c.step();
    assert_eq!(c.cpu().get_registers().get_c(), 0x42);
}
...
fn test_ld_hl_mem_roundtrip() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x99);
    c.step();
...
fn test_ldi_ldd() {
    ...
    c.cpu_mut().set_hl(0xC000);
    c.cpu_mut().get_registers_mut().set_a(0x11);
    c.step();
    assert_eq!(c.read_mem(0xC000), 0x11);
    assert_eq!(c.cpu().get_registers().get_hl(), 0xC001);
    c.step();
    assert_eq!(c.read_mem(0xC001), 0x11);
    assert_eq!(c.cpu().get_registers().get_hl(), 0xC000);
}
...
fn test_ldh_and_ld_c_indirect() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x5A);
    c.step(); // write A -> 0xFF80 (HRAM)
    assert_eq!(c.read_mem(0xFF80), 0x5A);
    c.cpu_mut().get_registers_mut().set_a(0x00);
    c.step(); // read 0xFF80 back into A
    assert_eq!(c.cpu().get_registers().get_a(), 0x5A);
}
...
fn test_add_a_flags() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x0F);
    c.cpu_mut().get_registers_mut().set_b(0x01);
    c.step();
...
fn test_add_a_carry_and_zero() {
    ...
    let mut c = setup(&[0x80]);
    c.cpu_mut().get_registers_mut().set_a(0xFF);
    c.cpu_mut().get_registers_mut().set_b(0x01);
    c.step();
    let r = c.cpu().get_registers();
...
fn test_adc_uses_carry_in() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x10);
    c.cpu_mut().get_registers_mut().set_flag_c(true);
    c.step();
    let r = c.cpu().get_registers();
...
fn test_logic_ops_flags() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0xF0);
    c.step();
    let r = c.cpu().get_registers();
    assert_eq!(r.get_a(), 0x00);
    assert!(r.get_flag_z());
...
fn test_inc_dec_r8_flags() {
    ...
    c.cpu_mut().get_registers_mut().set_b(0x0F);
    c.cpu_mut().get_registers_mut().set_flag_c(true);
    c.step();
    let r = c.cpu().get_registers();
    ...
    c.cpu_mut().get_registers_mut().set_b(0x00);
    c.step();
    let r = c.cpu().get_registers();
...
fn test_inc_hl_indirect() {
    ...
    assert!(c.cpu().get_registers().get_flag_z());
}
...
fn test_ld_hl_sp_e8() {
    ...
    c.cpu_mut().set_sp(0xD000);
    c.step();
    assert_eq!(c.cpu().get_registers().get_hl(), 0xD002);
    // SP itself is unchanged.
    assert_eq!(c.cpu().get_sp(), 0xD000);
}
...
fn test_rlca_clears_zero() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x80);
    c.cpu_mut().get_registers_mut().set_flag_z(true);
    c.step();
    let r = c.cpu().get_registers();
...
fn test_cb_rl_sets_zero_from_result() {
    ...
    c.cpu_mut().get_registers_mut().set_b(0x80);
    c.step();
    let r = c.cpu().get_registers();
...
fn test_cb_swap() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0xAB);
    c.step();
    assert_eq!(c.cpu().get_registers().get_a(), 0xBA);
}
...
fn test_cb_bit_set_res() {
    ...
    c.step(); // BIT 7, A
    assert!(c.cpu().get_registers().get_flag_z());
    assert!(c.cpu().get_registers().get_flag_h());
    c.step(); // SET 7, A
    assert_eq!(c.cpu().get_registers().get_a(), 0x80);
    c.step(); // RES 7, A
    assert_eq!(c.cpu().get_registers().get_a(), 0x00);
}
...
fn test_jr_taken_and_negative() {
    ...
    c.cpu_mut().get_registers_mut().set_flag_z(false);
    let cycles = c.step();
    assert_eq!(cycles, 12); // taken
    assert_eq!(c.cpu().get_pc(), 0x0004);
...
fn test_jp_and_jp_hl() {
    ...
    c.step();
    assert_eq!(c.cpu().get_pc(), 0x1234);

    // JP (HL)
    let mut c = setup(&[0xE9]);
    c.cpu_mut().set_hl(0x0200);
    c.step();
    assert_eq!(c.cpu().get_pc(), 0x0200);
}
...
fn test_call_ret() {
    ...
    c.step(); // CALL
    assert_eq!(c.cpu().get_pc(), 0x0006);
    assert_eq!(c.cpu().get_sp(), 0xCFFE);
    assert_eq!(c.read_mem(0xCFFE), 0x03); // low byte of 0x0003
    assert_eq!(c.read_mem(0xCFFF), 0x00);

    c.step(); // RET
    assert_eq!(c.cpu().get_pc(), 0x0003);
    assert_eq!(c.cpu().get_sp(), 0xD000);
}
...
fn test_daa_after_addition() {
    ...
    c.cpu_mut().get_registers_mut().set_a(0x45);
    c.cpu_mut().get_registers_mut().set_b(0x38);
    c.step();
    c.step();
    let r = c.cpu().get_registers();
...
fn test_interrupt_dispatch_priority() {
    ...
    c.cpu_mut().enable_interrupts();
    c.write_mem(0xFFFF, 0xFF); // IE: all enabled
    c.write_mem(0xFF0F, 0x06); // IF: LCDStat + Timer
    let cycles = c.step();
    assert_eq!(cycles, 20);
    assert_eq!(c.cpu().get_pc(), 0x0048); // LCDStat handler
    assert!(!c.cpu().ime_enabled()); // IME cleared on dispatch
...
fn test_ei_has_one_instruction_delay() {
    ...
    c.step(); // EI
    assert!(!c.cpu().ime_enabled());
    c.step(); // NOP (IME still off during this instruction)
    assert!(!c.cpu().ime_enabled() || c.cpu().get_pc() == 0x0002);
    let pc_after_nop = c.cpu().get_pc();
    assert_eq!(pc_after_nop, 0x0002);

    c.step(); // interrupt is now serviced
    assert_eq!(c.cpu().get_pc(), 0x0040);
}
...
fn test_halt_bug_executes_next_byte_twice() {
    ...
    assert_eq!(c.cpu().get_registers().get_a(), 0x01);
    assert_eq!(c.cpu().get_pc(), 0x0001);
    c.step(); // INC A again
    assert_eq!(c.cpu().get_registers().get_a(), 0x02);
    assert_eq!(c.cpu().get_pc(), 0x0002);
}
