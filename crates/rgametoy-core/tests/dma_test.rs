...
use rgametoy_core::bus::{Bus, MemoryBus};
...
/// Start an OAM DMA from `page` and advance the bus until the transfer is
/// active (past the one-M-cycle startup delay).
fn start_active_dma(page: u8) -> MemoryBus {
    let mut bus = MemoryBus::new();
...
fn vram_source_dma_leaves_the_external_bus_free() {
    let mut bus = start_active_dma(0x80); // source in VRAM
