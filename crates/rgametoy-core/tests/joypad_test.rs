...
fn joypad_irq(c: &Console) -> bool {
    c.read_mem(0xFF0F) & 0x10 != 0
}

fn clear_if(c: &mut Console) {
    c.write_mem(0xFF0F, 0x00);
}
...
fn selecting_a_group_with_a_held_button_interrupts() {
    let mut c = Console::new();
    c.set_buttons(press(RIGHT)); // hold Right
