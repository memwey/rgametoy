...
impl Ppu {
    ...
    /// available only under `--features debug` (and to this module's own tests).
    #[cfg(feature = "debug")]
...
// Timing tests that observe the *internal* mode/dot live here (not in
// `tests/`), so they read the private `mode`/`ly`/`dots` fields directly
// instead of forcing a public accessor. Behaviour-level PPU tests that go
// through the register / framebuffer interface stay in `tests/ppu_test.rs`.
#[cfg(test)]
mod tests {
    use super::*;

    /// A PPU with the LCD enabled so its state machine runs.
    fn enabled_ppu() -> Ppu {
        let mut ppu = Ppu::new();
    ...
    /// Length of mode 3 on line 2 (a normal line — line 0 after enable is
    /// special), measured against the internal OAM-scan→drawing→HBlank edges.
    fn mode3_len_on_line2(ppu: &mut Ppu) -> u32 {
        ...
        while ppu.ly == 2 {
            ...
            ppu.tick(1);
            dot += 1;
            if mode == PpuMode::OamScan && ppu.mode == PpuMode::Drawing {
                start = dot;
            }
            if mode == PpuMode::Drawing && ppu.mode == PpuMode::HBlank {
                len = dot - start;
            }
        }
        len
    ...
    /// Mode-3 length on line 2 with the given sprite OAM X positions (all at
    /// Y=18 i.e. screen line 2), sprites enabled.
    fn mode3_len_with_sprites(sprite_xs: &[u8]) -> u32 {
        let mut ppu = Ppu::new();
        for (i, &x) in sprite_xs.iter().enumerate() {
            let base = 0xFE00 + i as u16 * 4;
            ppu.write_oam(base, 18);
    ...
    /// OAM scan (mode 2) lasts exactly 80 dots on a normal line.
    #[test]
    fn oam_scan_lasts_80_dots() {
        let mut ppu = enabled_ppu();
        // Skip the special first line (no OAM scan, 452 dots); line 1 is normal.
        ppu.tick(200);
        ppu.tick(200);
        ppu.tick(52);
        assert_eq!(ppu.ly, 1);
        ...
        ppu.tick(79);
        assert_eq!(ppu.mode, PpuMode::OamScan, "still OAM scan after 79 dots");
        ppu.tick(1);
        assert_eq!(ppu.mode, PpuMode::Drawing, "80th dot enters pixel drawing");
    }

    /// The first line after enable starts in mode 0, goes straight to mode 3
    /// with no OAM scan, and is 4 dots short (LY reaches 1 at dot 452).
    #[test]
    fn first_line_after_enable_is_short_and_skips_oam_scan() {
        let mut ppu = enabled_ppu();
        assert_eq!(ppu.ly, 0);
        assert_eq!(ppu.mode, PpuMode::HBlank, "line 0 begins in mode 0");

        ppu.tick(80);
        assert_eq!(ppu.mode, PpuMode::Drawing, "drawing begins at dot 80, no OAM scan");
        ...
        assert_eq!(ppu.mode, PpuMode::OamScan, "line 1 is normal and scans OAM");
    }

    /// The STAT mode bits lag the internal transition by 4 dots out of a
    /// scan/blank mode.
    #[test]
    fn stat_mode_bits_lag_the_internal_transition() {
        let mut ppu = enabled_ppu();
        while ppu.ly != 2 {
            ppu.tick(1);
        }
        ppu.tick(80); // internal OAM-scan -> drawing on this dot
        assert_eq!(ppu.mode, PpuMode::Drawing, "internal mode is drawing");
        assert_eq!(ppu.read_register(0xFF41) & 3, 2, "STAT still reads mode 2");
        ppu.tick(4);
        assert_eq!(ppu.read_register(0xFF41) & 3, 3, "STAT catches up after 4 dots");
    }

    /// Stacked sprites at the same X pay the fetch-abort once: N at X=0 cost
    /// 11 + 6*(N-1) dots.
    #[test]
    fn stacked_sprite_penalty_aggregates() {
        ...
        assert_eq!(mode3_len_with_sprites(&[0, 0]), 189, "a second stacked sprite adds 6");
        assert_eq!(mode3_len_with_sprites(&[0, 0, 0]), 195, "a third stacked sprite adds 6");
    }
    ...
    /// At most 10 sprites are selected per line, so an 11th stacked sprite adds
    /// no further penalty.
    #[test]
    fn selects_at_most_10_sprites_per_line() {
